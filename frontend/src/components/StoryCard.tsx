...
const StoryCard = forwardRef(({ story, viewCommentOption, viewEditOption }: Props, ref) => {
    ...
    <Box ref={ref} sx={{ padding: '12px' }}>
      ...
      >
        ...
        <CardActions disableSpacing sx={{ mt: '8px', display: 'flex', justifyContent: 'center' }}>
          <IconButton aria-label="upvote" sx={{ p: '4px' }}>
            <BiUpvote />
          </IconButton>
          <Typography>{Number.isNaN(votes) ? 0 : votes}</Typography>
          <IconButton aria-label="downvote" sx={{ p: '4px' }}>
            <BiDownvote />
          </IconButton>
        ...
        <Box sx={{ mb: '20px', mt: '12px', display: 'flex', justifyContent: 'center', gap: '4px' }}>
          ...
          <CommentModal addCommentModal={addCommentModal} setAddCommentModal={setAddCommentModal} storyId={story._id} />
        </Box>
      </Card>
    </Box>
  );
